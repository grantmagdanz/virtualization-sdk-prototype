...
PLUGIN_IMPORTER_YAML = os.path.join(
    util_classes.PLUGIN_SCHEMAS_DIR, "plugin_importer.yaml"
)
...
class PluginImporter:
    ...
    def __import_in_subprocess(src_dir, module, entry_point, plugin_type, validate):
        ...
        queue = Queue()
        process = Process(
            target=_get_manifest,
            args=(queue, src_dir, module, entry_point, plugin_type, validate),
        )
        process.start()
        process.join()
    ...
    def __check_for_undefined_names(src_dir):
        ...
        style_guide = flake8.get_style_guide(
            select=["F821"], exclude=[exclude_dir], quiet=1
        )
        ...
        for checker in file_checkers:
            for result in checker.results:
                ...
                if result[0] == "F821":
                    msg = "{} on line {} in {}".format(
                        result[3], result[1], checker.filename
                    )
...
def _validate_and_get_manifest(module, module_content, entry_point):
    ...
    if not hasattr(module_content, entry_point):
        raise exceptions.UserError(
            ...
            " in module '{}'.".format(module, entry_point, entry_point, module)
        )
    plugin_object = getattr(module_content, entry_point)

    if plugin_object is None:
        raise exceptions.UserError(
            "Plugin object retrieved from the entry"
    ...
    manifest = {
        ...
        "hasLinkedStopStaging": bool(plugin_object.linked.stop_staging_impl),
        "hasLinkedStatus": bool(plugin_object.linked.status_impl),
        "hasLinkedWorker": bool(plugin_object.linked.worker_impl),
        "hasLinkedMountSpecification": bool(
            plugin_object.linked.mount_specification_impl
        ),
        "hasVirtualConfigure": bool(plugin_object.virtual.configure_impl),
        "hasVirtualUnconfigure": bool(plugin_object.virtual.unconfigure_impl),
        "hasVirtualReconfigure": bool(plugin_object.virtual.reconfigure_impl),
        ...
        "hasVirtualStatus": bool(plugin_object.virtual.status_impl),
        "hasInitialize": bool(plugin_object.virtual.initialize_impl),
    }
...
def _check_args(method_name, expected_args, actual_args):
    ...
    if len(expected_args) != len(actual_args):
        warnings.append(
            "Number of arguments do not match in method {}."
            " Expected: {}, Found: {}.".format(
                method_name, list(expected_args), str(actual_args)
            )
        )

    if not all(arg in expected_args for arg in actual_args):
        warnings.append(
            "Named argument mismatch in method {}."
            " Expected: {}, Found: {}.".format(
                method_name, list(expected_args), str(actual_args)
            )
        )
