...
@click.option(
    ...
    callback=click_util.validate_option_exists,
    help=(
        "Set the path to plugin config file. "
...
@click.option(
    ...
    help=(
        "Only generate the Python classes from the schema definitions. "
...
@click.option(
    ...
    hidden=True,
    help=(
        "An internal flag that does not enforce the format "
...
def build(plugin_config, upload_artifact, generate_only, skip_id_validation):
    ...
    with command_error_handler():
        build_internal.build(
            plugin_config, upload_artifact, generate_only, skip_id_validation
        )
...
@click.password_option(
    cls=click_util.PasswordPromptIf,
...
def download_logs(engine, plugin_config, user, password, directory):
    ...
    """
    with command_error_handler():
        download_logs_internal.download_logs(
            engine, plugin_config, user, password, directory
        )
