...
def entry_point_template():
    with open(
        os.path.join(init.PLUGIN_TEMPLATE_DIR, init.ENTRY_POINT_TEMPLATE_NAME), "r"
    ) as f:
        return f.read()
...
def direct_operations_template():
    with open(
        os.path.join(init.PLUGIN_TEMPLATE_DIR, init.DIRECT_OPERATIONS_TEMPLATE_NAME),
        "r",
    ) as f:
        return f.read()
...
def staged_operations_template():
    with open(
        os.path.join(init.PLUGIN_TEMPLATE_DIR, init.STAGED_OPERATIONS_TEMPLATE_NAME),
        "r",
    ) as f:
        return f.read()
...
def format_entry_point_template(entry_point_template):
    ...
    def format_template(plugin_name, ingestion_strategy, host_type):
        ...
        if ingestion_strategy == util_classes.DIRECT_TYPE:
            ...
            operations = jinja2.Environment().from_string(staged_operations_template())
...
class TestInitialize:
    ...
    @pytest.mark.parametrize(
        "ingestion_strategy", [util_classes.DIRECT_TYPE, util_classes.STAGED_TYPE]
    )
    @pytest.mark.parametrize(
        "host_type", [util_classes.UNIX_HOST_TYPE, util_classes.WINDOWS_HOST_TYPE]
    )
    def test_init(
        tmpdir,
        ingestion_strategy,
        ...
        result = plugin_util.read_and_validate_plugin_config_file(
            os.path.join(tmpdir.strpath, init.DEFAULT_PLUGIN_CONFIG_FILE), True, False
        )
        ...
        entry_file_path = os.path.join(tmpdir.strpath, config["srcDir"], entry_file)
    ...
    def test_init_without_plugin_name(tmpdir):
        init.init(
            tmpdir.strpath, util_classes.DIRECT_TYPE, "", util_classes.UNIX_HOST_TYPE
        )
    ...
    def test_plugin_from_init_is_valid(tmpdir, ingestion_strategy, plugin_name):
        init.init(
            tmpdir.strpath, ingestion_strategy, plugin_name, util_classes.UNIX_HOST_TYPE
        )
        ...
        validator = plugin_validator.PluginValidator(
            plugin_config_file, schema_file, True, True
        )
        validator.validate()
    ...
    def test_init_calls_cleanup_on_failure(
        mock_cleanup, mock_yaml_dump, tmpdir, plugin_name
    ):
        ...
        config_file_path = os.path.join(tmpdir.strpath, init.DEFAULT_PLUGIN_CONFIG_FILE)
        schema_file_path = os.path.join(tmpdir.strpath, init.DEFAULT_SCHEMA_FILE)
    ...
    def test_default_schema_definition(schema_template):
        ...
        assert (
            len(schema_template["repositoryDefinition"]["properties"]) == 1
        ...
        assert schema_template["repositoryDefinition"]["identityFields"] == ["name"]

        # Validate the source config schema only has the 'name' property.
        assert (
            len(schema_template["sourceConfigDefinition"]["properties"]) == 1
        ), json.dumps(schema_template["sourceConfigDefinition"])
        assert "name" in schema_template["sourceConfigDefinition"]["properties"]
        assert schema_template["sourceConfigDefinition"]["nameField"] == "name"
        assert schema_template["sourceConfigDefinition"]["identityFields"] == ["name"]
    ...
    def test_default_entry_point(plugin_id):
        ...
        raise RuntimeError(
            (
                "Failed to find assignment to variable '{}' "
                "in entry point template file."
            ).format(init.DEFAULT_ENTRY_POINT_SYMBOL)
        )
