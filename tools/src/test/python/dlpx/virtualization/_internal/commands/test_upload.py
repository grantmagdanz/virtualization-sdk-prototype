...
class FakeDelphixClient(object):
    ...
    def login(self, engine_api, user, password):
        if (
            engine_api.get("major") < 0
            or engine_api.get("minor") < 0
...
class TestUpload:
    ...
    @staticmethod
    @pytest.mark.parametrize("artifact_file", ["/not/a/real/file/upload_artifact.json"])
    ...
    @staticmethod
    def test_upload_password_incorrect(artifact_file, artifact_content, fake_client):
    ...
    def test_upload_plugin_failed(artifact_file, artifact_content, fake_client):
