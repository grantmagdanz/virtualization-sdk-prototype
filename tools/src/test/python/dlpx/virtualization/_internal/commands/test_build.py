...
class TestBuild:
    @staticmethod
    @mock.patch(
        "dlpx.virtualization._internal.plugin_util.get_plugin_manifest", return_value={}
    )
    ...
    @mock.patch.object(
        PluginValidator, "_PluginValidator__import_plugin", return_value=({}, None)
    )
    @mock.patch("dlpx.virtualization._internal.codegen.generate_python")
    ...
    def test_build_codegen_fail(
        ...
        mock_generate_python.side_effect = exceptions.UserError("codegen_error")
    ...
    def test_build_manifest_fail(
        ...
        # Raise UserError when codegen is called
        mock_plugin_manifest.side_effect = exceptions.UserError("manifest_error")
    ...
    @staticmethod
    @mock.patch(
        "dlpx.virtualization._internal.commands.build" ".prepare_upload_artifact"
    )
    @mock.patch(
        "dlpx.virtualization._internal.plugin_util.get_plugin_manifest", return_value={}
    )
    ...
    def test_build_prepare_artifact_fail(
        ...
        # Raise UserError when codegen is called
        mock_prep_artifact.side_effect = exceptions.UserError("prepare_artifact_error")
    ...
    @staticmethod
    @mock.patch(
        "dlpx.virtualization._internal.commands.build" ".generate_upload_artifact"
    )
    @mock.patch(
        "dlpx.virtualization._internal.plugin_util.get_plugin_manifest", return_value={}
    )
    ...
    @mock.patch("dlpx.virtualization._internal.codegen.generate_python")
    def test_generate_only_success(
        mock_generate_python,
        mock_prep_artifact,
        plugin_config_file,
        artifact_file,
    ...
    def test_get_linked_source_definition_type_direct(
        plugin_config_content, linked_source_def_type
    ):
        link_type = build.get_linked_source_definition_type(plugin_config_content)
        assert link_type == linked_source_def_type

    @staticmethod
    def test_prepare_discovery_definition(
        plugin_config_content, schema_content, discovery_definition
    ):
    ...
    @staticmethod
    def test_prepare_upload_artifact_success(
        basic_artifact_content, plugin_config_content, src_dir, schema_content
    ):
    ...
    def test_generate_upload_artifact_fail(artifact_file):
        ...
        message = err_info.value.message
        assert message == (
            "Failed to write upload_artifact file "
    ...
    def test_zip_and_encode_source_files_invalid_dir(src_dir):
        ...
        message = err_info.value.message
        assert message == (
            "Failed to read source code directory"
    ...
    def test_zip_and_encode_source_files_compileall_fail(mock_compile, src_dir):
        ...
        message = err_info.value.message
        assert message == (
            "Failed to compile source code in the directory" " {}.".format(src_dir)
        )
    ...
    def test_zip_and_encode_source_files_encode_fail(mock_encode, src_dir):
        ...
        message = err_info.value.message
        assert message == (
            "Failed to base64 encode source code in the"
...
class TestPluginUtil:
    @staticmethod
    @pytest.mark.parametrize(
        "plugin_config_file", ["/not/a/real/file/plugin_config.yml"]
    )
    @mock.patch("dlpx.virtualization._internal.codegen.generate_python")
    def test_no_plugin_file(mock_generate_python, plugin_config_file, artifact_file):
        with pytest.raises(exceptions.UserError) as err_info:
            build.build(plugin_config_file, artifact_file, False, False)
    ...
    @mock.patch("dlpx.virtualization._internal.codegen.generate_python")
    def test_plugin_bad_format(mock_generate_python, plugin_config_file, artifact_file):
        with pytest.raises(exceptions.UserError) as err_info:
            build.build(plugin_config_file, artifact_file, False, False)
    ...
    @mock.patch("dlpx.virtualization._internal.codegen.generate_python")
    def test_plugin_missing_fields(
    ...
    @staticmethod
    @mock.patch("dlpx.virtualization._internal.codegen.generate_python")
    def test_plugin_src_not_dir(
        mock_generate_python, plugin_config_file, artifact_file, src_dir
    ):
        ...
        message = err_info.value.message
        assert message == (
            "The path {} should be a" " directory but is not.".format(src_dir)
        )
    ...
    @staticmethod
    @mock.patch("dlpx.virtualization._internal.codegen.generate_python")
    def test_schema_file_bad_permission(
        mock_generate_python, plugin_config_file, artifact_file, schema_file
    ):
    ...
    @mock.patch("dlpx.virtualization._internal.codegen.generate_python")
    def test_plugin_missing_schema_def(
        mock_generate_python, plugin_config_file, artifact_file
    ):
        with pytest.raises(exceptions.UserError) as err_info:
            build.build(plugin_config_file, artifact_file, False, False)
    ...
    @pytest.mark.parametrize(
        ...
        [{"type": "objectxxx", "additionalProperties": False, "properties": {}}],
    )
    def test_plugin_extra_schema_def(
        mock_generate_python, plugin_config_file, artifact_file
    ):
        with pytest.raises(exceptions.UserError) as err_info:
            build.build(plugin_config_file, artifact_file, False, False)
    ...
    def test_manual_discovery_parameter(
        plugin_config_content, src_dir, schema_content, expected
    ):
        ...
        assert (
            expected
            == upload_artifact["discoveryDefinition"]["manualSourceConfigDiscovery"]
        )
