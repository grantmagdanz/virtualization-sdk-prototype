...
class TestClickUtil:
    ...
    def test_mutually_exclusive_options_success():
        @click.command()
        @click.option(
            "-a", cls=click_util.MutuallyExclusiveOption, mutually_exclusive=["b"]
        )
        @click.option(
            "-b", cls=click_util.MutuallyExclusiveOption, mutually_exclusive=["a"]
        )
    ...
    def test_validate_option_exists_without_value():
        ...
        with pytest.raises(click.BadParameter):
            click_util.validate_option_exists(
                Context(), click.Option(param_decls=["-a"]), None
            )
    ...
    def test_validate_option_exists_without_value_with_envvar_option():
        ...
        with pytest.raises(click.BadParameter) as excinfo:
            click_util.validate_option_exists(
                Context(), click.Option(param_decls=["-a"], envvar=envvar), None
            )
