...
class TestInitCli:
    ...
    def test_invalid_ingestion_strategy(plugin_name):
        ...
        result = runner.invoke(
            cli.delphix_sdk, ["init", "-n", plugin_name, "-s", "FAKE_STRATEGY"]
        )
...
class TestBuildCli:
    ...
    def test_default_plugin_file_success(
        mock_build, plugin_config_filename, plugin_config_content, artifact_file
    ):
        ...
        with runner.isolated_filesystem():
            with open(plugin_config_filename, "w") as f:
                f.write(yaml.dump(plugin_config_content, default_flow_style=False))

            plugin_config_file = os.path.realpath(f.name)
            result = runner.invoke(cli.delphix_sdk, ["build", "-a", artifact_file])

            assert result.exit_code == 0, "Output: {}".format(result.output)
            mock_build.assert_called_once_with(
                plugin_config_file, artifact_file, False, False
            )

    @staticmethod
    @mock.patch("dlpx.virtualization._internal.commands.build.build")
    def test_generate_only_success(
        mock_build, plugin_config_filename, plugin_config_content
    ):
        ...
        with runner.isolated_filesystem():
            ...
            assert result.exit_code == 0, "Output: {}".format(result.output)
            mock_build.assert_called_once_with(plugin_config_file, None, True, False)
    ...
    def test_valid_params(mock_build, plugin_config_file, artifact_file):
        runner = click_testing.CliRunner()
        result = runner.invoke(
            cli.delphix_sdk, ["build", "-c", plugin_config_file, "-a", artifact_file]
        )
    ...
    @mock.patch("dlpx.virtualization._internal.commands.build.build")
    def test_valid_params_new_name(mock_build, plugin_config_file, artifact_filename):
        runner = click_testing.CliRunner()
        result = runner.invoke(cli.delphix_sdk, ["build", "-c", plugin_config_file])

        assert result.exit_code == 0, "Output: {}".format(result.output)
        mock_build.assert_called_once_with(
            plugin_config_file,
            os.path.join(os.getcwd(), artifact_filename),
            False,
            False,
        )
    ...
    def test_skip_id_validation(mock_build, plugin_config_file, artifact_file):
        ...
        assert result.exit_code == 0, "Output: {}".format(result.output)
        mock_build.assert_called_once_with(
            plugin_config_file, artifact_file, False, True
        )
    ...
    def test_file_not_exist(plugin_config_file):
        ...
        assert result.output == (
            u"Usage: delphix-sdk build [OPTIONS]"
...
class TestUploadCli:
    ...
    def test_default_params(mock_upload, artifact_file):
        ...
        assert result.exit_code == 0, "Output: {}".format(result.output)
        mock_upload.assert_called_once_with(engine, user, artifact_file, password)
    ...
    @staticmethod
    @pytest.mark.parametrize("artifact_file", ["/not/a/real/file/artifact.json"])
    def test_file_not_exist(artifact_file):
        ...
        assert result.exit_code == 2
        assert result.output == (
            u"Usage: delphix-sdk upload [OPTIONS]"
    ...
    def test_with_config_file_success(
    ...
    @mock.patch("dlpx.virtualization._internal.commands.upload.upload")
    @pytest.mark.usefixtures("dvp_config_file")
    def test_with_config_file_override(
        mock_upload, artifact_file, dvp_config_properties
    ):
        engine = dvp_config_properties["engine"]
        user = "fake_admin"
    ...
    @staticmethod
    @pytest.mark.parametrize(
        "dvp_config_properties", [{"user": "user", "password": "password"}]
    )
    @pytest.mark.usefixtures("dvp_config_file")
...
class TestDownloadCli:
    @staticmethod
    @mock.patch("dlpx.virtualization._internal.commands.download_logs.download_logs")
    def test_default_params(mock_download_logs, plugin_config_file):
        ...
        mock_download_logs.assert_called_once_with(
            engine, plugin_config_file, user, password, directory
        )
    ...
    def test_missing_params():
        ...
        result = runner.invoke(
            cli.delphix_sdk,
            ["download-logs", "-u", user, "-d", directory, "--password", password],
        )
    ...
    @pytest.mark.usefixtures("dvp_config_file")
    def test_with_config_file_success(
        mock_download_logs, plugin_config_file, dvp_config_properties
    ):
        engine = dvp_config_properties["engine"]
        user = dvp_config_properties["user"]
        password = dvp_config_properties["password"]
        ...
        assert result.exit_code == 0, "Output: {}".format(result.output)
        mock_download_logs.assert_called_once_with(
            engine, plugin_config_file, user, password, cwd
        )
    ...
    def test_with_config_file_override(
        ...
        try:
            os.chdir(os.path.dirname(plugin_config_file))
            runner = click_testing.CliRunner()
            result = runner.invoke(cli.delphix_sdk, ["download-logs", "-u", user])
        finally:
            os.chdir(cwd)
